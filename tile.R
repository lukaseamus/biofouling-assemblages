...
tricell <- tile$Tricellaria_inopinata
...
ggplot(tile, aes(mat, tricell)) +
  geom_boxplot()

ggplot(tile, aes(ciona, tricell)) +
  geom_point()
...
leveneTest(tricell~depth*ori*mat)
...
shapiro.test(tricell) # non-normal
...
logtricell <- log(tricell+1)
shapiro.test(logtricell) # non-normal
...
wilcox.test(tricell~depth)
wilcox.test(tricell~mat) # significant *
wilcox.test(tricell~ori)
...
# material significantly affects the abundance of Tricellaria inopinata
...
triplot <- ggplot(tile, aes(mat, tricell)) +
  geom_boxplot(aes(fill = mat), width = .3, colour = "grey50") +
  scale_fill_manual(values = c("#fddca3", "#f5a54a"), guide = F) +
  geom_jitter(width = .12, alpha = .5, colour = "grey50", size = 2) +
  xlab("\nMaterial") +
  ylab(expression(atop("Abundance of "*italic("Tricellaria inopinata")))) +
  mytheme
...
grid.arrange(botplot, ausplot, triplot, ncol = 3)
...
plym <- data.frame(lon = -4.131199, lat = 50.365790)

Plymouth <- ggmap(get_map(plym, zoom = 16, maptype = "toner-lite")) + 
  coord_map()+ theme(plot.background = element_blank(),
                     panel.background = element_blank(),
  ...
  xlab("Longitude") + ylab("Latitude") +
  geom_point(x = -4.131199, y = 50.365790, 
             size = 3, fill = "#fbb36d", colour = "#ef3a4d", pch = 21) +
...
Britain <- ggplot(UK)+
  ...
  geom_point(x = -4.131199, y = 50.365790, 
           size = 3, fill = "#fbb36d", colour = "#ef3a4d", pch = 21) +
  annotate("segment", x = -6, xend = -3.15, y = 49.5, yend = 49.5) +
  annotate("text", label = "200 km", x = -4.57, y = 49, size = 5, family = "Helvetica")
